...
void ReadPageGuard::Flush() {
  // UNIMPLEMENTED("TODO(P1): Add implementation.");
  // If the page is not dirty, there's nothing to flush.
  if (!frame_->is_dirty_) {
    return;
  }

  // 1. The promise's template argument must be <bool> to match the DiskRequest struct.
  std::promise<bool> promise;
  std::future<bool> future = promise.get_future();

  // 2. Create a vector, as the Schedule method expects a vector of requests.
  std::vector<DiskRequest> requests;

  // 3. Initialize the DiskRequest correctly with is_write=true and move the promise.
  requests.emplace_back(DiskRequest{true, const_cast<char *>(frame_->GetData()), page_id_, std::move(promise)});

  // 4. Pass the vector of requests to the scheduler.
  disk_scheduler_->Schedule(requests);

  // 5. Wait for the background worker thread to complete the disk write.
  future.get();

  // 6. After a successful flush, the page is no longer dirty.
...
WritePageGuard::WritePageGuard(WritePageGuard &&that) noexcept {
  ...
  // Invalidate 'that' so its destructor does nothing
  that.is_valid_ = false;
  that.frame_ = nullptr;
}
...
void WritePageGuard::Flush() {
  ...
  // 2. Create a promise/future pair to wait for the I/O to complete.
  std::promise<bool> promise;
  std::future<bool> future = promise.get_future();

  // 3. Create a vector for the disk request.
  std::vector<DiskRequest> requests;
  requests.emplace_back(DiskRequest{true, frame_->GetDataMut(), page_id_, std::move(promise)});

  // 4. Schedule the write request.
  disk_scheduler_->Schedule(requests);

  // 5. Wait for the write to finish.
  future.get();

  // 6. The page in memory now matches the page on disk, so it's no longer dirty.
  frame_->is_dirty_ = false;
}
